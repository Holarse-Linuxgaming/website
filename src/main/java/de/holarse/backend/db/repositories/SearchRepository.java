...
public interface SearchRepository extends JpaRepository<SearchIndex, Integer> {
    ...
    @Query(value = "select nodeid as nodeid, ptitle as title, purl as url from mv_searchindex " +
                   ...
                   "and document @@ websearch_to_tsquery('german', :query)", nativeQuery = true)
    Page<SearchResultView> searchTags(@Param("tags") final String tags, @Param("query") final String query, final Pageable pageable);    
